...
| Commercial Readiness Auditor | `agents/commercial-readiness-auditor.md` | G14 상용화 판정: 5기준(항목/저장/전송/텍스트/디자인) × 운영 실측 keeper, backup→deploy→remeasure→rollback (v2.1.0) |
...
| **2026-08-30** | **G15 hardening extended 5→7 checks + operator-approval honesty (v2.2.1)** | scripts/hardening-check.js + src/__tests__/hardening-check.test.js + knowledge_base/00_SECURITY_STANDARDS.md §7.1 + _harness_test/track31-hardening/ + CHANGELOG.md | 실제 coolhanx.com 하드닝 런의 교훈을 게이트에 반영. H6 자동화도구(python-requests/HeadlessChrome/curl/Go/Scrapy) 차단 확인, H7 검색색인 보존(Googlebot/Bingbot/Yeti/Daum/facebookexternalhit는 반드시 200 — 과차단=SEO 회귀=FAIL, H1의 균형추). H2 지문헤더(X-Powered-By/x-nextjs-cache) 수정을 operator-approval로 재분류: 프레임워크/서버가 앱 미들웨어 이후 재부착하므로 앱단 삭제·레포 next.config.js는 배포스크립트가 서버설정을 안 보내면 무효 → 서버파일 변경(웹서버 Header unset OR 서버 next.config.js)=금지구역. finding에 fix_class 부착 + /_next/ 자산경로로 Next.js는 어차피 드러남(은폐 아닌 마무리) 정직 노트. AI 에이전트 UA에 OAI-SearchBot+GPTBot 추가. Tests 14/14 (total 203/203), track31 재실행 0 false +/-. coolhanx.com 잔여 X-Powered-By 1건은 operator-approval-pending(서버파일, 무인 수정 금지). |
| **2026-08-29** | **G15 Production edge/bot hardening probe (v2.2.0)** | scripts/hardening-check.js + src/__tests__/hardening-check.test.js + knowledge_base/00_SECURITY_STANDARDS.md §7 + agents/(security-reviewer step 3.6, commercial-readiness-auditor pre-check) + references/ai-native-sdlc-map.md + _harness_test/track31-hardening/ + CHANGELOG.md | 사용자가 coolhanx.com에서 발견한 실제 구멍 5개를 재실행 가능한 게이트로 전환(운영에 실HTTP 측정): H1 AI 에이전트 UA 우회(허용목록이 차단목록보다 먼저 검사돼 ChatGPT-User·Claude-User·PerplexityBot이 200 HTML 수신 — API는 막혔으나 웹층은 뚫림), H2 스택 지문 헤더(X-Powered-By·x-nextjs-cache·server:Express), H3 프레임워크 404 노출(Express "Cannot GET /api"→중립 404), H4 UA 정합성(크롬 주장하나 sec-ch-ua+sec-fetch-mode 둘 다 없으면 차단), H5 robots.txt가 AI 답변/에이전트 크롤러 차단. 각 검사 PASS/FAIL/NOT_RUN, FAIL은 측정값(미들웨어/robots/nginx 소스 판독 후 결함 판별), 도달 불가→NOT_RUN(거짓 PASS 없음). nginx는 자동수정 금지구역 유지. Tests 10/10 (total 199/199), track31 실HTTP 적대적: 취약 오리진→5개 전부 FAIL 명명, 하드닝 오리진→PASS, 0 false +/-. |
| **2026-08-07** | **G14 Commercial-Readiness Gate — 상용화 판정 (v2.1.0)** | scripts/commercial-gate.js + agents/commercial-readiness-auditor.md (new) + src/__tests__/commercial-gate.test.js + CLAUDE.md (team table) + skills/coolhan-development-orchestrator (SKILL.md G14 step) + references/ai-native-sdlc-map.md + _harness_test/track30-commercial/ + CHANGELOG.md | 사용자 지정 기능: 외부 연동 유무가 아닌 5개 사용자 대면 기준(항목 저장·재조회/부분·중복·동시 저장 무결성/전송 도착+실패 안내/DE·EN·KO 텍스트/실브라우저 디자인)으로 상용화 가부 판정. 기준마다 영구 지킴이(반복 실행 스크립트)를 config에 선언 — 지킴이 없는 칸은 NOT_RUN이지 통과가 아님(C10). 운영 base_url 실HTTP 프로브 기록, keeper는 실자식프로세스 실행, 산출물 _workspace/COMMERCIAL_VERDICT_<date>.md(①가부 ②차단+증거tail+측정≠결함 원칙 ③미측정 정직 신고), 종료코드 READY 0/BLOCKED 1/NOT_READY 4. 감사 규율은 agent P0: 운영 실측만 증거·소스 판독 후 결함 판별·백업→배포→재측정→롤백·운영 테스트데이터 금지·nginx/docker/SECRET_KEY 금지구역 STOP. Tests 7/7 (total 189/189), track30 적대적(실keeper+coolhanx.com 실HTTP 302 프로브): 혼합→BLOCKED+NOT_RUN 정직, 전커버→READY, 0 false +/-. |
| **2026-08-07** | **G13 Nonstop supervisor — 무중단 개발의 바깥 루프 (v2.0.0)** | scripts/nonstop.js + src/__tests__/nonstop.test.js + skills/coolhan-development-orchestrator (SKILL.md self-resume §G13) + references/ai-native-sdlc-map.md + _harness_test/track29-nonstop/ + CHANGELOG.md | 사용자 목표 "roop 무중단 개발"의 마지막 갭: G10은 유닛 내부 루프, G11은 세션 조기종료 차단이지만, 세션이 진짜 끝나면(컨텍스트 소진/크래시) 바톤 붙여넣기는 여전히 인간 몫이었음. nonstop.js는 터미널에서 1회 실행하는 감독 프로세스: completion-check exit 0까지 헤드리스 세션(claude -p ... bypassPermissions, 템플릿 설정 가능)을 자동 재기동. 정지는 5개 명명 조건+구분 종료코드로만(COMPLETE 0→G12 보고서 자동 생성 / CMD_ERROR 2 / STOP_APPROVED 3 / NO_PROGRESS 4→3세션 무변화 시 정직 중단, 웨징 런에 무한 과금 방지 / MAX_SESSIONS 5→기본 50). 진행 판정은 세션 종료코드가 아닌 실제 백로그 diff, 세션별 증거는 _nonstop-log.jsonl, 기록된 ESCALATE는 세션 0회 기동. Tests 9/9 (total 182/182), track29 실프로세스 적대적: 3유닛→정확히 3세션 COMPLETE, 깨진 세션 명령→NO_PROGRESS(원본 에러 tail), ESCALATE→0기동; 0 false +/-. |
| **2026-08-07** | **G12 Run Observability Report (v1.9.0)** | scripts/run-report.js + src/__tests__/run-report.test.js + skills/coolhan-development-orchestrator (SKILL.md completion-gate G12 step) + references/ai-native-sdlc-map.md + _harness_test/track28-runreport/ + CHANGELOG.md | 루프 로드맵의 Foundation/Observability 층: G5 원장·G10 루프 상태·G8-B 백로그·제안·디자인 이력이 각자 조각만 기록하고 아무것도 런 전체 그림을 합성하지 않았음. run-report.js가 한 아티팩트로 합성: 백로그 완료율(남은/미검증 유닛 명명), 게이트별 PASS/FAIL/NOT_RUN(–run-id 필터), 유닛별 이터레이션·ESCALATE·마지막 실패 tail, 반복 교훈(≥2×), 대기 제안 수. 관찰자 전용 — 판정 불변경·비차단, 빈 워크스페이스는 "기록 없음 ≠ 완료 증명" 정직 보고. 엔진 완료 게이트에 연결(최종 5줄 보고에 경로 인용), 중간 "어디까지 됐어?" 질문에도 경로 전달. Tests 6/6 (total 173/173), track28 적대적: 현실적 중간상태→완료율/필터/교훈/ESCALATE/제안 정확(구분선 과계수 결함 트랙 중 발견·수정), 빈 상태→정직 캐비앳, 0 false +/-. |
| **2026-08-07** | **G11b Ask-Guard + G11c Run-Armer — bypassPermissions에서도 멈추는 마지막 2개 경로 차단 (v1.8.1)** | .claude/hooks/(ask-guard.js, run-armer.js) + .claude/settings.json (PreToolUse matcher AskUserQuestion + UserPromptSubmit) + src/__tests__/ask-run-guards.test.js + skills/coolhan-development-orchestrator (SKILL.md ARM step 개정) + references/ai-native-sdlc-map.md + _harness_test/track27-askguard/ + CHANGELOG.md | 사용자 보고: 권한 무시로 설정해도 멈춰서 물어봄 — 권한 모드는 원인이 아니었음. 갭 2개: (1) AskUserQuestion은 턴을 끝내지 않고 세션을 대기시키므로 Stop 훅이 못 봄 → ask-guard가 PreToolUse에서 미완 런 중 질문을 deny(기본값 선택+DECISIONS.md+계속 지시 재주입; _stop-approved.json은 정당 경로로 유지). (2) _run-active.json 아밍 자체가 프로즈 지시였음 → run-armer가 사용자의 쿨한 연속개발 명령을 감지해 하네스가 직접 아밍(점검/업데이트 프롬프트는 비아밍), 이전 런의 stale 승인 자동 제거. Tests 7/7 (total 167/167), track27 실호출 적대적 5 시나리오 0 false +/-. |
| **2026-08-07** | **G11 Stop-Guard hook — harness-level loop enforcement (v1.8.0)** | .claude/hooks/stop-guard.js + .claude/settings.json (hooks → real Claude Code schema; old metadata → coolhanHooksDoc) + src/__tests__/stop-guard.test.js + skills/coolhan-development-orchestrator (SKILL.md engine-loop ARM step) + references/ai-native-sdlc-map.md + _harness_test/track26-stopguard/ + CHANGELOG.md | User-reported: 루프를 안 돌고 계속 멈춰서 물어봄 — 모든 기존 대책(규칙 8·4조건 게이트·UNIT PREAMBLE)은 모델이 자발적으로 따라야 하는 프로즈였음. Stop 훅은 모델 바깥(하네스)에서 턴 종료 시점에 실행: 활성 런(_run-active.json) + 백로그 미완(completion-check≠0)이면 정지를 차단하고 남은 유닛+무질문 규칙+정당한 정지 경로를 재주입. 정당한 정지는 _stop-approved.json 기록으로 허용(4조건/P0/ESCALATE/파괴적 작업), 완료 시 마커 자동 회수, 25회 안전밸브+fail-open으로 사용자를 가두지 않음, 마커 없는 일반 세션은 완전 무영향. 발견: 기존 settings.json hooks 블록은 Claude Code 스키마가 아니어서 한 번도 실행된 적 없음 → 실스키마로 재구성. Tests 9/9 (total 160/160), track26 실호출 적대적: 일반세션 무영향/미완런 차단+카운터/승인정지 허용/완료시 마커 회수, 0 false +/-. |
